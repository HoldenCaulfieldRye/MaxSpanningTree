...
def ReadFile(filename):
    f = open(filename);
    # function int converts a string to an integer
    noVariables = int(f.readline())      # 1st line contains # variables
    noRoots = int(f.readline())          # 2nd second line contains # root nodes
    ...
    noStates = map(int,((f.readline()).split()))  # 3rd line contains # states of each variable
    noDataPoints = int(f.readline())              # 4th line contains # of data points
...
def AppendList(filename, aList):
    f = open(filename, 'a')                  # 'a' opens file for appending
