...
def weightSort(weights):
    try: return sorted(weights.keys(), key = lambda edge: -1*weights[edge])
...
if __name__ == '__main__':
    ...
    AppendString('results.txt','Dependency matrix for the HepatitisC :\n')
    AppendString('results.txt','Dependency matrix for the HepatitisC data set:\n')
    AppendString('results.txt',MI)
    AppendString('results.txt','\n\n')

    depList = DependencyList(MI)

    AppendString('results.txt','Dependency list for the HepatitisC data set:\n')
    AppendString('results.txt',depList)
    AppendString('results.txt','\n\n')
    
    tree = SpanningTreeAlgorithm(depList, noVariables)
    AppendString('results.txt','Spanning tree found for the HepatitisC data set: \n\n')    
    AppendString('results.txt',tree)
